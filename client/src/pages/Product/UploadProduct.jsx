...
import sweetAlertSuccess from "../../utils/SweetAlertSuccess";
...
const UploadProduct = () => {
  ...
  const handleSubmit =async(e)=>{
   try {
    e.preventDefault()
    setloading(true)
    const createone = await axios.post(Endpoints.createproduct.url,data,{withCredentials:true});
    if(createone){
      
      sweetAlertSuccess(createone?.data?.message);
      setdata({
        _id: "",
        name: "",
        image: [],
  ...
  return (
    <section>
      ...
      <div>
        <form className="px-3 py-4 flex flex-col gap-1" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-wrap  gap-2 cursor-pointer mb-4">
              {data?.image &&
                data?.image?.map((img, index) => (
                  <div className="border relative border-gray-200 group rounded-sm mt-3 flex justify-center items-center h-[120px] w-[120px] ">
                    <img
                      onClick={() => {
                        setopenImage(true);
                        setImageURL(img);
