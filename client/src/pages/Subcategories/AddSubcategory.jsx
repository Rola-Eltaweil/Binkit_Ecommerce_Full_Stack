...
import { getsubcategories } from "../../Redux/feature/categorySlice";
const AddSubcategory = ({ onclose, fetchSubcategory }) => {
  ...
  const checkform = subcategorydata.name && subcategorydata.image;
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const createSubcategory = await axios.post(
        Endpoints.AddSubcategory.url,
        subcategorydata,
        { withCredentials: true }
      );
      if (createSubcategory) {
        toast.success(createSubcategory.data.message);
        onclose();
        setsubcategorydata({
          name: "",
          image: "",
        ...
        fetchSubcategory();
        dispatch(
          getsubcategories([...subcategory, createSubcategory?.data?.data])
        );
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <section className="inset-0 bg-neutral-900 bg-opacity-60 fixed z-50 flex items-center justify-center p-4 ">
      <div className="bg-white max-w-xl w-full p-3 rounded-sm">
        <div className="flex items-center justify-between">
          <p className="text-[16px] font-semibold mt-2"> Add Sub Category</p>
        ...
        <form className="flex flex-col gap-2 " onSubmit={handleSubmit}>
          ...
          <div
            className={`${
              !checkform ? `bg-gray-400` : `bg-primary-200 hover:bg-primary-100`
