...
const OpenEditcategory = ({ onclose, fetchData, categoryData }) => {
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault();
      setloading(true);
      const updated = await axios.put(Endpoints.updatecategory.url, data, {
        withCredentials: true,
      });
      if (updated) {
    ...
    <section className="top-0 bottom-0 right-0 left-0 bg-neutral-800 bg-opacity-50 fixed flex items-center justify-center">
      <div className="bg-white  max-w-3xl w-full px-3 py-5 rounded-md ">
        <div className=" flex items-center justify-between">
          <p className="text-[16px] font-semibold"> Update Category</p>
        ...
        <form className="flex flex-col gap-2 " onSubmit={handleSubmit}>
          ...
          <div className="bg-primary-200 flex justify-center py-1.5 rounded-sm mt-2">
...
export default OpenEditcategory;
